...
# We are going to write a program that will ask the user to input an arbitrary
# number of integers, store them in a collection, and then dempnstrate how the
...
while count < target_int:
    ...
    # Notice bellow I use ==, which is different from =. The single equals is an
...
# A for loop is using a local copy of the value. Changes inside the loop
# will be forgotten when the loop terminates.
print("Using a for loop")
for value in ints:
    print(str(value))

# The while loop is directly accessing elements. Changes inside of it will be
# kept when the loop terminates.
